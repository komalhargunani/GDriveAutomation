...
import com.automation.utils.Utils;
	 ...
	 *
	 */
	public class SaveAfterEditGdrivePointFileTest extends TestBase {
		
		/**
	     * Calling the base class constructor to initialize the configuration
	     * 
	     */
		public SaveAfterEditGdrivePointFileTest() {
			super();		
		}
		...
		public void verifyAfterEditsaveSheetFile(String fileName){
			...
			try {
				if((Utils.getExtention(fileName)).equalsIgnoreCase("ppt"))
				{
					...
					Assert.assertNotNull(EditedPage.getEditedText("ppt"));
					...
					Assert.assertNotNull(EditedPage.getEditedText("pptx"));
				}
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
