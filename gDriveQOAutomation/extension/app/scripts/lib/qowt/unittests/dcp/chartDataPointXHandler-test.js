...
define([
  ...
  describe('chart data point X DCP Handler', function() {
    ...
    it('should ignore the DCP if its element-type is not dpX', function() {
      var v = {
        el: {
          etp: 'blah'
        }
    ...
    it('should handle an undefined DcpModel.dcpHandlingChart', function() {
      ...
      var dataPointXDCP = FIXTURES.dataPointX();
      var result = handler.visit({el: dataPointXDCP});
    ...
    it('should successfully cache a data point in a series in ChartsModel',
        function() {
          var xVal = 18;
          var dataPointXDCP = FIXTURES.dataPointX(xVal);
          var result = handler.visit({el: dataPointXDCP});
          ...
          expect(dataPoint[0]).toBe(1);
          expect(dataPoint[1]).toBe(xVal);
        });

    it('should successfully cache multiple data points in a series in ' +
        'ChartsModel', function() {
          var xVal1 = 24;
          var dataPointXDCP = FIXTURES.dataPointX(xVal1);
          var result = handler.visit({el: dataPointXDCP});
          expect(result).toBeUndefined();
          var xVal2 = 49;
          dataPointXDCP = FIXTURES.dataPointX(xVal2);
          result = handler.visit({el: dataPointXDCP});
          expect(result).toBeUndefined();
          var xVal3 = 75;
          dataPointXDCP = FIXTURES.dataPointX(xVal3);
          result = handler.visit({el: dataPointXDCP});
          expect(result).toBeUndefined();
          ...
          expect(dataPoint1[0]).toBe(1);
          expect(dataPoint1[1]).toBe(xVal1);
          var dataPoint2 = targetSeries[1];
          expect(dataPoint2).toBeDefined();
          expect(dataPoint2[0]).toBe(2);
          expect(dataPoint2[1]).toBe(xVal2);
          var dataPoint3 = targetSeries[2];
          expect(dataPoint3).toBeDefined();
          expect(dataPoint3[0]).toBe(3);
          expect(dataPoint3[1]).toBe(xVal3);
        });
  });
});
