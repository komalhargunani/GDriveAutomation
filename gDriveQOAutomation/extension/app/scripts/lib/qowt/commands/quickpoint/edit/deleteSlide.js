...
/**
 * @fileoverview qowt command to delete slides
 *
...
define([
  ...
  var _factory = {
    /**
     * Creates a new deleteSlide command and returns it.
     *
     * @param {Object} command command data for the command
     * @return {Object} delete command object
     */
    create: function(command) {
      // don't try to execute if it's missing crucial data
      if (command === undefined) {
        throw new Error('Delete slide command missing command data');
      }
      if (command.slideNumbers === undefined) {
        throw new Error('Delete slide cmd missing slideNumbers');
      }
      ...
      var module = function() {
        var _api = CommandBase.create('deleteSld', optimistic, callsService);
        ...
        _api.changeHtml = function() {
          ThumbnailStrip.deleteSlides(command.slideNumbers);
        };
